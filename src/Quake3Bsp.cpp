...
void CQuake3BSP::GenerateTexture() {
    ...
    unsigned char* image =
        stbi_load("textures/_engine/missing.png", &width, &height, &num_channels, 3); // use stb to load the missing texture
    ...
    for (int i = 0; i < m_numOfTextures; i++) {
        unsigned char* image = stbi_load(tname[i], &width, &height, &num_channels, 3);
        // filter and arrange
        if (image) {
